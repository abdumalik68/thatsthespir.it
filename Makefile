...
mysql-dump:
	...
	@docker exec $(shell docker-compose ps -q mysqldb) mysqldump $(MYSQL_DATABASE) -u"$(MYSQL_ROOT_USER)" -p"$(MYSQL_ROOT_PASSWORD)" > $(MYSQL_DUMPS_DIR)/db-dump-$(today).sql 2>./make.log
	@make resetOwner

mysql-restore:
	@docker exec -i $(shell docker-compose ps -q mysqldb) mysql -u"$(MYSQL_ROOT_USER)" -p"$(MYSQL_ROOT_PASSWORD)" $(MYSQL_DATABASE) < $(MYSQL_DUMPS_DIR)/importthis.sql 2>./make.log
